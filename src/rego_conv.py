...
class RegoConverter:
    ...
    def is_numeric_value(self, value: str) -> bool:
        ...
        if value is None or value == '':
            return False
        try:
            float(str(value).replace('m', '').replace('k', '').replace('d', '').replace(';', '').strip())
            return True
        except:
            return False
...
"""
    ...
    def generate_config_rule(self, row: Dict[str, Any]) -> str:
        ...
        if has_value_list:
            ...
            if self.is_numeric_value(inverse_val):
                condition = f"input.config.{config_key_sanitized} == {inverse_val}"
            else:
                condition = f"lower(input.config.{config_key_sanitized}) == \"{inverse_val.lower()}\""
            msg_text = f"Запрещено значение {config_key} = {inverse_val}"
        else:
            # Обычная проверка
            if expected_value:
                # Очистка значения от точек с запятой и пробелов
                clean_value = expected_value.replace(';', '').strip()
                
                if self.is_numeric_value(clean_value):
                    condition = f"input.config.{config_key_sanitized} != {clean_value}"
                                           ...
                                           'log_line_prefix']:
                    # Проверка через contains для сложных строк
                    condition = f"not contains(lower(input.config.{config_key_sanitized}), \"{clean_value.lower()}\")"
                else:
                    condition = f"lower(input.config.{config_key_sanitized}) != \"{clean_value.lower()}\""
...
deny[msg] {{
    ...
    {condition}
    msg := "[{category}] {req_name}: {msg_text} [{req_id}]"
}}
"""
        return rule
    ...
    def generate_file_permission_rule(self, row: Dict[str, Any]) -> List[str]:
        ...
        if expected_permissions:
            rule_perm = f"""
deny[msg] {{
    input.files["{file_path_pattern}"]
    input.files["{file_path_pattern}"].permissions != "{expected_permissions}"
...
deny[msg] {{
    input.files["{file_path_pattern}"]
    input.files["{file_path_pattern}"].owner != "{expected_owner}"
...
deny[msg] {{
    ...
    not any_header_contains(input.config.add_header, "{header_name}")
    msg := "[{category}] {req_name}: Ожидалось add_header {header_name} [{req_id}]"
}}
"""
                        rules.append(rule)
